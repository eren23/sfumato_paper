...
a different seed, at matched length ($8{,}000$ steps), and compute
AR-seed0 $\to$ AR-seed1: $0.329$.
...
\midrule
AR-seed0 $\to$ AR-seed1 (same mode, diff.\ seed) & $0.329$ \\
...
\noindent The cross-mode alignment ($0.169$) sits well \emph{below}
the same-mode baseline ($0.329$): cross-mode is only $+0.045$ above
chance, whereas the same-mode baseline is $+0.205$ above chance. Going
from same-mode to cross-mode therefore loses $\sim\!78\%$ of the
above-chance feature alignment ($1 - 0.045/0.205$). This is the
rigorous statement of the result: \textbf{the two heads share
substantially less feature structure than two independently-trained
SAEs of the \emph{same} head do.} (An earlier $2{,}000$-step seed-1
SAE gave a more conservative $0.286$; the matched $8{,}000$-step
retrain reported here \emph{widened} the gap, as predicted, so $0.329$
is the figure we report.) The pre-committed ``$<\!0.4$ = specialise''
threshold is thus empirically anchored: the relevant comparison is the
$0.329$ same-mode baseline, not an arbitrary cut.
...
\paragraph{P.4b --- the matched-magnitude control.}
Divergence alone is not evidence the \emph{direction} matters: any
large enough perturbation moves greedy generation. We therefore hold
magnitude fixed (unit direction $\times\,k{=}4$) and compare the
mean token-divergence (over $6$ prompts, AR mode) of the AR-feature
direction against $4$ random unit directions and a diff-feature
direction:

\begin{center}
\begin{tabular}{lr}
\toprule
steering direction & mean divergence \\
\midrule
AR-feature \#$8435$ & $0.258$ \\
diff-feature \#$8435$ & $0.308$ \\
random unit dirs ($n{=}4$) & $0.155 \pm 0.035$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent Learned feature directions move generation $\sim\!1.7\times$
more than a matched-magnitude random direction, well outside the
random spread ($0.258$ vs $0.155 \pm 0.035$). So the steered
directions carry generation-relevant signal --- the effect is
\textbf{direction-specific, not generic perturbation}. We stop short
of claiming full directed causation of \emph{mode-specific} behaviour:
the effect is moderate, and a diff-mode direction injected into AR
generation is in fact \emph{more} disruptive than the AR feature
(plausibly because it is off-manifold for the AR head). The honest
reading is that steering is \emph{controlled-and-consistent-with}, not
\emph{proof-of}, the specialisation story.
