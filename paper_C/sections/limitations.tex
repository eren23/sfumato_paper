...
causal-steering evidence (P.4/P.4b) is \emph{controlled but moderate}:
against a matched-magnitude random-direction null, learned feature
directions move generation $\sim\!1.7\times$ more ($0.258$ vs
$0.155 \pm 0.035$ mean divergence), so the effect is direction-specific
rather than generic perturbation --- but the magnitude is modest and a
diff-mode direction is itself disruptive in AR mode, so we frame
steering as controlled-and-consistent-with, not proof-of, directed
causation. True causal mediation (activation patching / feature
...
\begin{enumerate}
  ...
  \item Move from steering to true causal mediation (activation
        patching / feature ablation) to test whether the specialised
        features \emph{cause} mode-specific behaviour, not just
        correlate with it.
