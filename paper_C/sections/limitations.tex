...
$0.169$ against a same-mode different-seed baseline ($0.286$), which
is the correct null; but that second SAE was trained for $2{,}000$
steps vs.\ $8{,}000$ for the originals, so $0.286$ is a conservative
(low) estimate of same-mode alignment --- a matched-length retrain is
the clean follow-up and would, if anything, widen the specialisation
gap. The null is measured at the pre-head residual (\code{ln\_f})
...
\begin{enumerate}
  ...
  \item Retrain the same-mode null-baseline SAE at matched length
        ($8{,}000$ steps) and add random-direction / diff-feature
