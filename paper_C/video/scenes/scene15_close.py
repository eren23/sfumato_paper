...
Fix: tighter line spacing inside the F10 panel; shrink body text and
explicit aligned_edge=LEFT so hypothesis text does not collide with
the same-3B-token line.
"""
...
def _panel(title_str, lines, color, w=5.6, h=4.7):
    box = RoundedRectangle(width=w, height=h, corner_radius=0.2, color=color,
                           stroke_width=2, fill_opacity=0.08)
    head = body_text(title_str, size=18, color=color, weight=BOLD)
    body = VGroup(*[body_text(line, size=13, color=FG) for line in lines]).arrange(DOWN, aligned_edge=LEFT, buff=0.16)
    inner = VGroup(head, body).arrange(DOWN, aligned_edge=LEFT, buff=0.28).move_to(box.get_center())
    ...
    def construct(self) -> None:
        ...
        left = _panel(
            ...
            ["same arch as F9: 305M composite",
             "same 3B token budget, but:",
             "load_mixed_tokens =",
             "  95 % FineWeb-Edu +",
             "  5 % formatted GSM8K Q/A",
             "",
             "hypothesis: accuracy moves",
             "off the 0 % free-run floor;",
             "model learns the \"Answer:\"",
