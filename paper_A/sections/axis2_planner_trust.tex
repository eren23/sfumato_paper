\section{Phenomenon 2: Planner-Content Trust Is Capacity-Dependent in Opposite Directions Across Planners}
\label{sec:axis2}
...
This is a previously-unmeasured \emph{phenomenon}: LoRA capacity
differentially affects how much the model \emph{trusts} the
content of an upstream plan, and the direction of the effect is
opposite for the two planners.  We have two capacity rungs (v2 at
$10$M and v3 at $22$M trainable parameters); the data establishes
that the inversion is real, but it does not yet trace a fitted
non-monotonic curve.  We label this a phenomenon rather than an
axis on the same footing as
brittleness~(\S\ref{sec:axis1}) and
diversity~(\S\ref{sec:axis3}) for that reason; a third capacity
rung (e.g.\ $5$M or $50$M) and per-planner multi-seed runs would
upgrade it.
